...
def write_notes(root, version, codename):
    notes_dir = os.path.join(root, "tron-release", "release-notes")
    ...
    path = os.path.join(notes_dir, f"tv_{version}.md")
    with open(path, "w", encoding="utf-8") as handle:
        handle.write(text)
    ...
    def run_main_with(self, error):
        with mock.patch.object(cli, "cmd_prepare", side_effect=error):
            ...
            with contextlib.redirect_stderr(captured):
                result = cli.main(["prepare", "deadbeef", "--build-run-id", "1"])
...
class TestLoadConfig(unittest.TestCase):
    ...
    subscript deep inside `cmd_prepare`. Every test below writes its own
        temp-file config -- never the real `tron-release/config.json`.
    """
    ...
    def test_environment_overrides_keep_fork_rehearsal_isolated(self):
        with tempfile.TemporaryDirectory() as root:
            write_config(root, "tronprotocol/solidity", [])
            with mock.patch.dict(os.environ, {
                "TRON_RELEASE_REPO": "yanghang8612/solidity",
                "TRON_RELEASE_SOLC_BIN_REPO": "yanghang8612/solc-bin",
                "TRON_RELEASE_SIGNING_KEY_FINGERPRINT": "A" * 40,
                "TRON_RELEASE_QA_REVIEWERS": "qa-one, qa-two",
            }):
                config = cli.load_config(root)
        self.assertEqual(config["repo"], "yanghang8612/solidity")
        self.assertEqual(config["solcBinRepo"], "yanghang8612/solc-bin")
        self.assertEqual(config["signingKeyFingerprint"], "A" * 40)
        self.assertEqual(config["qaReviewers"], ["qa-one", "qa-two"])
    ...
    def test_g1_runs_before_g2(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            args = argparse.Namespace(
                commit=COMMIT, build_run_id=BUILD_RUN_ID,
                release_ref="origin/develop", workdir=root, dry_run=True,
            )
...
class TestCmdPrepareDryRun(unittest.TestCase):
    """End-to-end `prepare --dry-run`: every git/gh call is faked."""
    ...
    def setUp(self):
        ...
        self.fixtures = {
            "solc-macos": f"MachO junk {VERSION} junk {SHORT_COMMIT} junk".encode(),
            "solc-static-linux": b"#!/bin/sh\necho fake solc linux\n",
            "solc-static-linux-arm": f"ELF arm junk {VERSION} junk {SHORT_COMMIT} junk".encode(),
    ...
    def build_run(self):
        table = {
            ...
            ("gh", "run", "view", BUILD_RUN_ID, "--repo", REPO,
             "--json", "databaseId,headSha,conclusion,workflowName"):
                json.dumps({
                    "databaseId": int(BUILD_RUN_ID), "headSha": COMMIT,
                    "conclusion": "success",
            ...
            (os.path.join(self.artifacts_dir, "solc-static-linux"), "--version"):
        ...
        for name in self.fixtures:
            table[("gh", "run", "download", BUILD_RUN_ID, "--repo", REPO,
                   "--name", artifact_names[name], "--dir", self.artifacts_dir)] = ""
    ...
    def test_writes_manifest_and_skips_gh_release_create(self):
        ...
        args = argparse.Namespace(
            commit=COMMIT, build_run_id=BUILD_RUN_ID,
            release_ref="origin/develop", workdir=self.root, dry_run=True,
        )
