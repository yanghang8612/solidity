...
def g3_artifacts_present(present: List[str]) -> None:
    """G3: all four release binaries must exist under the release commit's S3 prefix."""
    missing = [name for name, _, _ in ARTIFACT_SPECS if name not in present]
    if missing:
        raise GateError(f"G3: missing artifacts in S3: {missing}")


_EXECUTABLE_ARTIFACTS = ("solc-macos", "soljson.js")
_STRING_ARTIFACTS = ("solc-static-linux", "solc-windows.exe")
...
def g7_ready_to_mail(is_draft: bool, solcbin_pr_url: Optional[str]) -> None:
    """G7: the release request email may only go out while nothing is public yet.

    The email is a *request*, not an announcement: when it is sent, no tag
    exists yet (GitHub only creates one on publish), the release itself must
    still be a draft, and the solc-bin PR must be open but unmerged. That
    ordering is what makes an unattended send safe -- the worst case of a
    wrongly-sent email is embarrassment, never a poisoned CDN.
    ...
    if not is_draft:
        raise GateError(
            "G7: release is already published; the request email must precede publication"
        )
