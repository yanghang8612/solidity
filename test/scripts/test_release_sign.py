...
from release import checksums, cli
...
def download_argv(workdir, tag=TAG, repo=REPO):
    patterns = []
    for name in ARTIFACT_NAMES + ["shasum.txt"]:
        patterns += ["-p", name]
...
def upload_argv(sig_paths, tag=TAG, repo=REPO):
    return ["gh", "release", "upload", tag, "--repo", repo, "--clobber"] + sig_paths
...
class TestVerifyDownloaded(unittest.TestCase):
    ...
    of exactly the four release artifacts.
    """
    ...
    def test_accepts_matching_digests_for_all_four(self):
    ...
    def test_rejects_incomplete_coverage(self):
        """Signing three of four must not silently succeed."""
        with self.assertRaisesRegex(GateError, "soljson.js"):
...
class TestCmdSign(unittest.TestCase):
    ...
    real `gpg`. The workdir is pre-seeded with the four artifacts and a
    ...
    def setUp(self):
        ...
        self.sig_paths = [os.path.join(self.workdir, name) + ".sig" for name in ARTIFACT_NAMES]
    ...
    def build_run(self, include_upload):
        ...
        if include_upload:
            table[tuple(upload_argv(self.sig_paths))] = ""
    ...
    def test_uploads_signatures_when_not_dry_run(self):
        ...
        self.assertEqual(result, 0)
        self.assertIn(upload_argv(self.sig_paths), run.calls)
